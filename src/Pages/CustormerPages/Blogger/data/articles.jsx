import articleImage1 from "../../../../assets/imageBlogger/article1.jpg";
import articleImage2 from "../../../../assets/imageBlogger/article2.jpg";
import articleImage3 from "../../../../assets/imageBlogger/article3.jpg";
import skincareImage from "../../../../assets/imageBlogger/skincare.jpg";

export const articlePost = [
  {
    title: "Mỹ phẩm hữu cơ là gì?",
    description: "Top 5 mỹ phẩm hữu cơ được yêu thích nhất",
    content:
      "Sử dụng các loại mỹ phẩm hữu cơ đã trở thành một trào lưu được nhiều người đón nhận bởi các cam kết về sức khỏe và bảo vệ môi trường. Đây cũng là một trong những xu hướng làm đẹp mới được nhiều tín đồ mê skincare.",
    image: skincareImage,
  },
  {
    title: "Mỹ phẩm vô cơ là gì?",
    description: "Top 10 mỹ phẩm hữu cơ được yêu thích nhất",
    content:
      "Sử dụng các loại mỹ phẩm hữu cơ đã trở thành một trào lưu được nhiều người đón nhận bởi các cam kết về sức khỏe và bảo vệ môi trường. Đây cũng là một trong những xu hướng làm đẹp mới được nhiều tín đồ mê skincare.",
    image: skincareImage,
  },
  {
    title: "Mỹ phẩm hữu cơ và vô cơ là gì?",
    description: "Top 1000 mỹ phẩm hữu cơ được yêu thích nhất",
    content:
      "Sử dụng các loại mỹ phẩm hữu cơ đã trở thành một trào lưu được nhiều người đón nhận bởi các cam kết về sức khỏe và bảo vệ môi trường. Đây cũng là một trong những xu hướng làm đẹp mới được nhiều tín đồ mê skincare.",
    image: skincareImage,
  },
];

export const articlesDiscovery = [
  {
    date: "20 tháng 6, 2024",
    title: "Kem chống nắng Nhật Bản tốt không?",
    description: "Top 5 kem chống nắng Nhật Bản được yêu thích",
    content:
      "Kem chống nắng Nhật Bản luôn được đánh giá cao bởi chất lượng. Vậy đâu là top 5 kem chống nắng Nhật Bản được yêu thích nhất hiện nay? Hãy cùng Beauty Love tìm hiểu trong bài viết dưới đây...",
    views: 220,
    image: articleImage1,
  },
  {
    date: "20 tháng 6, 2024",
    title: "Chống nắng Sunplay Skin Aqua",
    description: "Top 5 kem chống nắng Sunplay Skin Aqua đáng mua",
    content:
      "Sunplay Skin Aqua là dòng kem chống nắng được ưa chuộng tại Nhật Bản và nhiều quốc gia châu Á, trong đó có Việt Nam. Các nàng thường hay phân vân không biết bản thân mình phù hợp với loại kem chống nắng nào của nhà Sunplay...",
    views: 220,
    image: articleImage2,
  },
  {
    date: "20 tháng 6, 2024",
    title: "Muối tắm sữa bò A Bonne của nước nào",
    description: "Sử dụng có tốt không? Top 5 sản phẩm tốt của hãng",
    content:
      "Việc sử dụng các loại muối tắm đang dần trở nên phổ biến hiện nay bởi nhiều lợi ích mà các sản phẩm này đem lại. Trong đó, muối tắm sữa bò A Bonne là một sản phẩm chăm sóc da được nhiều người yêu thích và sử dụng...",
    views: 220,
    image: articleImage3,
  },
  {
    date: "20 tháng 6, 2024",
    title: "Muối tắm sữa bò A Bonne của nước nào",
    description: "Sử dụng có tốt không? Top 5 sản phẩm tốt của hãng",
    content:
      "Việc sử dụng các loại muối tắm đang dần trở nên phổ biến hiện nay bởi nhiều lợi ích mà các sản phẩm này đem lại. Trong đó, muối tắm sữa bò A Bonne là một sản phẩm chăm sóc da được nhiều người yêu thích và sử dụng...",
    views: 220,
    image: articleImage3,
  },
  {
    date: "20 tháng 6, 2024",
    title: "Kem chống nắng Nhật Bản tốt không?",
    description: "Top 5 kem chống nắng Nhật Bản được yêu thích",
    content:
      "Kem chống nắng Nhật Bản luôn được đánh giá cao bởi chất lượng. Vậy đâu là top 5 kem chống nắng Nhật Bản được yêu thích nhất hiện nay? Hãy cùng Beauty Love tìm hiểu trong bài viết dưới đây...",
    views: 220,
    image: articleImage1,
  },
  {
    date: "20 tháng 6, 2024",
    title: "Chống nắng Sunplay Skin Aqua",
    description: "Top 5 kem chống nắng Sunplay Skin Aqua đáng mua",
    content:
      "Sunplay Skin Aqua là dòng kem chống nắng được ưa chuộng tại Nhật Bản và nhiều quốc gia châu Á, trong đó có Việt Nam. Các nàng thường hay phân vân không biết bản thân mình phù hợp với loại kem chống nắng nào của nhà Sunplay...",
    views: 220,
    image: articleImage2,
  },
];

export const healthArticles = [
  {
    title: "THƯƠNG HIỆU NATURE'S BOUNTY...",
    description:
      "Nature's Bounty là một thương hiệu dinh dưỡng nổi tiếng tại Hoa Kỳ, được thành lập ...",
    date: "20 tháng 6, 2024",
    views: 220,
    image: articleImage1,
  },
  {
    title: "THỰC PHẨM GIÀU COLLAGEN...",
    description:
      "Top 10 loại thực phẩm giàu collagen giúp cho làn da trẻ đẹp bạn không nên bỏ qua...",
    date: "20 tháng 6, 2024",
    views: 220,
    image: articleImage2,
  },
  {
    title: "COLLAGEN DHC CỦA NHẬT...",
    description:
      "Collagen DHC của Nhật có tốt không? Nên uống dạng nước hay dạng viên...",
    date: "20 tháng 6, 2024",
    views: 220,
    image: articleImage3,
  },
];
